...
# Import passwords from the secret file
...
# with st.form(key = 'passwords-form', clear_on_submit = True):
	...
	if (submitted and passwords is not None):

		raw_text = passwords.getvalue().decode('utf-8').splitlines()
		no_of_lines = len(passwords.getvalue().decode('utf-8').splitlines())

		config = {}
		config_keys = ['LINKAHEAD_URL', 'LINKAHEAD_USERNAME', 'LINKAHEAD_PASSWORD', 'UMG_PROXY', 'AMAZON_S3_BUCKET', 'AMAZON_S3_ACCESS_KEY', 'AMAZON_S3_SECRET_KEY', 'AMAZON_S3_ENDPOINT_URL']

		for i, key in enumerate(config_keys):
			start_index = raw_text[i].find("'") + 1
			end_index = raw_text[i].rfind("'")
			config[key] = raw_text[i][start_index:end_index]
		...
		try:
			...
			url = config['LINKAHEAD_URL'],
			password_method = "plain",
			ssl_insecure = True, # remove after naming server
			username = config['LINKAHEAD_USERNAME'],
			password = config['LINKAHEAD_PASSWORD'],
			timeout = 1000
			)
			...
			try:
				...
				username = config['LINKAHEAD_USERNAME'],
				password = config['LINKAHEAD_PASSWORD'],
				https_proxy = config['UMG_PROXY'],
				timeout = 1000
				)

			except:
				
				ErrorMessage = st.error('Unsuccessful connection with the Linkahead DB. Check the passwords file or contact the admin(s) for help.', icon = None)
...
with placeholder.form(key = 'LSM_SCAN_FORM_KEY', clear_on_submit = True):
	...
	if passwords is not None:
		...
		if submitted:
			...
			access_key = config['AMAZON_S3_ACCESS_KEY']
			secret_key = config['AMAZON_S3_SECRET_KEY']
			bucket_name = config['AMAZON_S3_BUCKET']
			gwdg_client = boto3.client('s3', endpoint_url = config['AMAZON_S3_ENDPOINT_URL'], aws_access_key_id = access_key, aws_secret_access_key = secret_key)
			gwdg = boto3.resource('s3', endpoint_url = config['AMAZON_S3_ENDPOINT_URL'], aws_access_key_id = access_key, aws_secret_access_key = secret_key)
			...
			for i in range(len(tiff_files)):
				...
				if i == 0:
					ProgressBarText.caption("{}% image uploaded ({} images out of {} images)".format(int(100*(i+1)/len(tiff_files)), i+1, len(tiff_files)))
			...
			SuccessMessageImagesUpload = st.success('Successfully uploaded all images. Application will reset now.')
